...
void QcNeedleItem::draw(QPainter *painter)
{
    ...
    case QcNeedleItem::CompassNeedle:
        ...
        break;
    case QcNeedleItem::PitchNeedle:
    {createPitchNiddle(getRadius(tmpRect));
        grad.setStart(mNeedlePoly[0]);
        grad.setFinalStop(mNeedlePoly[1]);
        grad.setColorAt(0.9,Qt::red);
        grad.setColorAt(1,Qt::blue);
    ...
    case QcNeedleItem::RollNeedle:
        createRollNiddle(getRadius(tmpRect));
        grad.setStart(mNeedlePoly[0]);
        grad.setFinalStop(mNeedlePoly[1]);
        grad.setColorAt(0.9,Qt::red);
        grad.setColorAt(1,Qt::blue);
...
{
    QVector<QPointF> tmpPoints;
    tmpPoints.append(QPointF(0.0, r/1.5));
...
void QcNeedleItem::createPitchNiddle(float r)
{
    QVector<QPointF> tmpPoints;

    tmpPoints.append(QPointF(-r/1.2, r/3.0));
    tmpPoints.append(QPointF(r/1.2, r/3.0));
    tmpPoints.append(QPointF(r/3.0, -r/3.0));
    tmpPoints.append(QPointF(-r/3.0,-r/3.0));
...
void QcNeedleItem::createRollNiddle(float r)
{
    QVector<QPointF> tmpPoints;
    tmpPoints.append(QPointF(0.0, r/2));
    tmpPoints.append(QPointF(-r/1.2, r/2));
    tmpPoints.append(QPointF(-r/3,-r/2));
    tmpPoints.append(QPointF(r/3,-r/2));
    tmpPoints.append(QPointF(r/1.2,r/2));
    ...
    QcItem(parent)
{
    mPitch = 0;
    mRoll = 0;
   setBrush(QColor(127,187,191));
   setBrush2(QColor(191,191,191));
   setBrush3(Qt::darkBlue);
}
...
QColor QcAttitudeMeter::getBrush() const
{
    return m_brush;
}

void QcAttitudeMeter::setBrush(const QColor &brush)
{
    m_brush = brush;
}

QColor QcAttitudeMeter::getBrush2() const
{
    return brush2;
}

void QcAttitudeMeter::setBrush2(const QColor &value)
{
    brush2 = value;

}

QColor QcAttitudeMeter::getBrush3() const
{
    return m_brush3;

}

void QcAttitudeMeter::setBrush3(const QColor &brush3)
{
    m_brush3 = brush3;
    update();
}
...
void QcAttitudeMeter::draw(QPainter *painter)
{
   ...
   // drawPitchSteps(painter,tmpRect);
    ...
    painter->setPen(pen);
    painter->drawLine(QPoint(tmpRect.center().x(),tmpRect.topLeft().y()),QPoint(tmpRect.center().x(),tmpRect.bottomRight().y()));
    painter->drawLine(QPoint(tmpRect.topLeft().x(),tmpRect.center().y()),QPoint(tmpRect.bottomRight().x(),tmpRect.center().y()));
...
qDebug()<<"called";
    ...
    QColor clr1 = getBrush();
    QColor clr2 = getBrush2();
    clr1.setAlphaF(0.5);
...
void QcAttitudeMeter::drawLowerEllipse(QPainter *painter, const QRectF &tmpRect)
{
    ...
    QColor clr1 = QColor(127,187,191);
    QColor clr2 = QColor(191,191,191);
    radialGrad2.setColorAt(0, getBrush3());
    radialGrad2.setColorAt(.8,getBrush3());
